...
export class RestaurantDetailsComponent implements OnInit {
    ...
    if (this.selectedRestaurant.placeId !== "") {
      ...
      this.restaurantService.service.getDetails(request, (place, status) => {
        ...
        const scrollElement = document.querySelector(
          `.restaurant-${this.restaurantService.selectedRestaurant.placeId}`
        );
  ...
  addRating(pseudo: string = "Inconnu", comment: string) {
    ...
    this.selectedRestaurant.rating = this.restaurantService.setRatingAverage(
      this.restaurantService.selectedRestaurant.ratings
    );
    this.activeFilter = 0;
    this.mouseOut();
  }
